...
class VolatilityDBSCANAnalyzer:
    ...
    def __init__(self, eps=0.5, min_samples=5, vl_weight=0.5):
        """
        ...
        vl_weight (float): Weight for volatility label features (0.0 to 1.0)
        """
        self.eps = eps
        ...
        self.vl_encoded_columns = ['vl_high', 'vl_medium', 'vl_low']
        self.all_feature_columns = self.feature_columns + self.vl_encoded_columns
    ...
    def load_and_prepare_data(self, input_file):
        ...
        try:
            ...
            missing_cols = [col for col in self.feature_columns + ['vl'] if col not in df.columns]
            if missing_cols:
                raise ValueError(f"Missing required columns: {missing_cols}")

            # Remove rows with missing values in feature columns
            initial_rows = len(df)
            df = df.dropna(subset=self.feature_columns + ['vl'])
    ...
    def encode_volatility_labels(self, df):
        """
        Encode volatility labels (vl) into numerical features with specified weighting

        Parameters:
        df (pd.DataFrame): Input dataframe with 'vl' column

        Returns:
        np.ndarray: Feature matrix with encoded vl features
        """
        # One-hot encode volatility labels
        df_encoded = df.copy()
        df_encoded['vl_high'] = (df_encoded['vl'] == 'high').astype(int)
        df_encoded['vl_medium'] = (df_encoded['vl'] == 'medium').astype(int)
        df_encoded['vl_low'] = (df_encoded['vl'] == 'low').astype(int)

        # Extract numerical features
        numerical_features = df_encoded[self.feature_columns].values

        # Extract encoded vl features
        vl_features = df_encoded[self.vl_encoded_columns].values

        # Apply weighting: numerical features get (1 - vl_weight), vl features get vl_weight
        numerical_weight = 1 - self.vl_weight

        # Combine features with weighting
        weighted_numerical = numerical_features * numerical_weight
        weighted_vl = vl_features * self.vl_weight

        # Concatenate weighted features
        combined_features = np.concatenate([weighted_numerical, weighted_vl], axis=1)

        print(f"Feature weighting applied:")
        print(f"  Numerical features weight: {numerical_weight:.2f}")
        print(f"  Volatility label weight: {self.vl_weight:.2f}")
        print(f"  Combined feature matrix shape: {combined_features.shape}")

        return combined_features, df_encoded

    def train_dbscan_model(self, df, save_models=True):
        """
        Train DBSCAN model on the prepared data with weighted features
        ...
        """
        print("\n" + "=" * 50)
        print("TRAINING DBSCAN MODEL WITH WEIGHTED FEATURES")
        print("=" * 50)

        # Encode volatility labels and combine with numerical features
        X_combined, df_encoded = self.encode_volatility_labels(df)

        print(f"All features used: {self.all_feature_columns}")
        print(f"Weighted feature matrix shape: {X_combined.shape}")

        # Normalize/Scale the combined features
        print("\nScaling weighted features...")
        X_scaled = self.scaler.fit_transform(X_combined)

        print("Feature scaling statistics for combined features:")
        for i, col in enumerate(self.all_feature_columns):
        ...
        if 'vl' in df_result.columns:
            ...
            # Show detailed cluster-volatility analysis
            print(f"\nDetailed cluster-volatility analysis:")
            for cluster_id in sorted(df_result['cluster'].unique()):
                ...
                vl_distribution = cluster_data['vl'].value_counts(normalize=True) * 100

                print(f"\n{cluster_name} ({len(cluster_data)} points):")
                for vl_type in ['high', 'medium', 'low']:
                    percentage = vl_distribution.get(vl_type, 0)
                    print(f"  {vl_type} risk: {percentage:.1f}%")

        # Save models (including encoded features info)
        if save_models:
            self.save_models()
    ...
    def predict_cluster(self, row_data):
        """
        Predict cluster for a single row of data using weighted features
        ...
        # Encode volatility labels and combine with numerical features
        features_combined, _ = self.encode_volatility_labels(temp_df)
...
def main():
    ...
    # Initialize analyzer with 50% weight for volatility labels
    analyzer = VolatilityDBSCANAnalyzer(eps=0.3, min_samples=3, vl_weight=0.5)
