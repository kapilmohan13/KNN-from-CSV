...
class LSTMVolatilityPredictor:
    ...
    def build_lstm_model(self, input_shape: Tuple[int, int]) -> Sequential:
        ...
        # Use string names instead of metric objects to avoid serialization issues
        model.compile(
    ...
    def load_model(cls, model_dir: str = "lstm_model") -> 'LSTMVolatilityPredictor':
        ...
        # Load model with custom objects to handle potential serialization issues
        try:
            instance.model = load_model(model_path)
        except Exception as e:
            print(f"Warning: Error loading model with default method: {e}")
            print("Attempting to load with compile=False and recompile...")
            instance.model = load_model(model_path, compile=False)

            # Recompile the model with safe metric names
